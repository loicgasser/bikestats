# code that provides the logic for server.R
# adapted from *velopassBirdsEye.R"
...
withByWeekAndMonth <- function(tbd, varDateTime="day"){
	...
	tbd$year <- floor_date(tbd[, varDateTime], unit="year")
	tbd
}
...
##stations
stations <- union(unique(vlp2010$start_statn), unique(vlp2010$end_statn))

aggregatedHourlyUsage <- function(hourlyTrips, aggBy="year") {
	hourlyOut.agg <- data.frame(ddply(hourlyTrips, c("start_prd", "wkdayOrEnd", aggBy), 
																		.fun="nrow"),
															drxn="out")
	names(hourlyOut.agg) <- c("hour", "wkdayOrEnd", aggBy, "num.trips", "drxn")
	hourlyIn.agg <- data.frame(ddply(hourlyTrips, c("end_prd", "wkdayOrEnd", aggBy), 
																	 .fun="nrow"),
														 drxn="in")
	names(hourlyIn.agg) <- c("hour", "wkdayOrEnd", aggBy, "num.trips", "drxn")
	hourly.agg <- rbind(hourlyOut.agg, hourlyIn.agg)
	hourly.agg
}
