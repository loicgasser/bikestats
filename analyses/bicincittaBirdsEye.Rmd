---
title: 'Analysis Change of the Bicincitta data: the Bird''s Eye View'
output:
  pdf_document:
    fig_caption: yes
    keep_tex: yes
...
datapath <- paste(prjpath, "data/bicincitta/", sep="")
...
```
#Introduction
We look at the higher level properties of the Bicincitta data.

#Data
The original data was JSON files, that we have saved as CSVs. We do not have trips in the data, but transactions.
```{r readdata}
dd <- datapath
bicinTrxns <- read.csv(paste(dd, "Bicincitta_transactions_20150430.csv", sep=""),
									 	stringsAsFactors=FALSE)
```
There are two columns that contain information about when the data entry was created, which we will drop. We wil, convert the event time to a date-time object, and use to order the data,

```{r cleanandorder}
bicinTrxns$event_time <- parse_date_time( bicinTrxns$event_time, "mdy_hms")
bicinTrxns$created_at <- NULL
bicinTrxns$updated_at <- NULL
bicinTrxns$drxn <- gsub(x=bicinTrxns$direction, pattern="check-", replacement="")
bicinTrxns$direction <- NULL
bicinTrxns <- bicinTrxns[ order(bicinTrxns$event_time),]
```

We will convert the direction to a numeric value,
```{r numericdrxn}
numDrxn <- c("in"=-1, "out"=1)
bicinTrxns$drxn <- numDrxn[bicinTrxns$drxn]
```

We need to convert the transaction data to trips.
